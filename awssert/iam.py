...
class PolicyAssertions:
    ...
    def name(self, policy, name):
        assert name == policy.policy_name

    @positives("has")
    @negatives("does_not_have")
    def description(self, policy, description):
        assert description == policy.description
    ...
    def created_at(self, policy, date):
        if not isinstance(date, datetime):
            date = dateutil.parser.parse(date)
        assert date == policy.create_date
    ...
    def last_updated_at(self, policy, date):
        if not isinstance(date, datetime):
            date = dateutil.parser.parse(date)
        assert date == policy.update_date
    ...
    def attached_to(self, policy, *entities):
        ...
        assert all([entity in attached[type(entity).__name__] for entity in entities])
